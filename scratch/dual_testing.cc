...
main (int argc, char *argv[])
{
  ...
  STA_wifiNodes.Create (2*user_num);
  
  NodeContainer leftSTA_nodes;
  NodeContainer rightSTA_nodes;
  for (int i = 0; i < user_num; i++) {
    leftSTA_nodes.Add (STA_wifiNodes.Get (2*i));
    rightSTA_nodes.Add (STA_wifiNodes.Get (2*i+1));
  }
  ...
  for (int i = 0; i < user_num; i++) {
    leftSTA_p2ppairs.push_back(NodeContainer (leftSTA_nodes.Get (i),STA_etherNodes.Get (i)));
    rightSTA_p2ppairs.push_back(NodeContainer (rightSTA_nodes.Get (i),STA_etherNodes.Get (i)));
  }
...
/* Set Analytical Codebook for the DMG Devices */
  ...
  for (int i = 0; i < user_num; i++) {
    leftSTA_wifiDevice.Add (wifi.Install (wifiPhy_60480, left_WifiMac, leftSTA_nodes.Get (i)));
  }
  ...
  for (int i = 0; i < user_num; i++) {
    rightSTA_wifiDevice.Add (wifi.Install (wifiPhy_64800, right_WifiMac, rightSTA_nodes.Get (i)));
  }
  ...
  for (uint32_t i = 0; i < STA_wifiNodes.GetN(); i+=2) {
    if (i == 0){
      positionAlloc->Add (Vector (-2.0, 0.0, 0.0));   /* DMG STA0*/
      positionAlloc->Add (Vector (-2.0, 0.0, 0.0));   /* DMG STA0*/
    }
    else{
  ...
  sta0_mobility.push_back(STA_etherNodes.Get (0)->GetObject<ConstantVelocityMobilityModel>());
  sta0_mobility.push_back(STA_wifiNodes.Get (0)->GetObject<ConstantVelocityMobilityModel>());
  sta0_mobility.push_back(STA_wifiNodes.Get (1)->GetObject<ConstantVelocityMobilityModel>());
  ...
  cout << "position of STA0's left mmwave: " << STA_wifiNodes.Get (0)->GetObject<ConstantVelocityMobilityModel>()->GetPosition () << endl;
  cout << "position of STA0's right mmwave: " << STA_wifiNodes.Get (1)->GetObject<ConstantVelocityMobilityModel>()->GetPosition () << endl;
  ...
  if (pcapTracing)
    {
      ...
      wifiPhy_60480.EnablePcap ("Traces/AccessPoint_"+comment, ap_wifiDevice, false);
      wifiPhy_60480.EnablePcap ("Traces/StaNode_"+comment, leftSTA_wifiDevice, false);

      wifiPhy_64800.SetPcapDataLinkType (YansWifiPhyHelper::DLT_IEEE802_11_RADIO);
      wifiPhy_64800.EnablePcap ("Traces/AccessPoint_"+comment, ap_wifiDevice, false);
      wifiPhy_64800.EnablePcap ("Traces/StaNode_"+comment, rightSTA_wifiDevice, false);
  ...
  if (activateApp)
    {
      ...
      Simulator::Schedule (Seconds (2.1), &left_new_CalculateThroughput, file_dir,leftAPWifi_node,STA_etherNodes,left_apps,staWifiMac_left,true);
      Simulator::Schedule (Seconds (2.1), &right_new_CalculateThroughput, file_dir,rightAPWifi_node,STA_etherNodes,right_apps,staWifiMac_right,false);
    }
