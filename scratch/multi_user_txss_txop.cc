...
NetDeviceContainer staDevices[user_num];

Ptr<DmgApWifiMac> apWifiMac;
Ptr<DmgStaWifiMac> staWifiMac[user_num];
...
void RapidBeamTraining()
{
  // cout << "executing Rapid Beam Training" << endl;
  // staWifiMac[0]->Perform_TXSS_TXOP (apWifiMac->GetAddress ());
  Simulator::Schedule (Seconds (0.1), &RapidBeamTraining);
}

void 
InstallPacketSink (string dataRate, Ptr<Node> apNode, Ptr<Node> staNode, Ipv4Address sta_address, string socketType, double simulationTime, int user_i)
{
  /* Install packet sink on the STA node for Rx*/
  cout << "Installing packet sink" << endl;
  PacketSinkHelper sinkHelper (socketType, InetSocketAddress (Ipv4Address::GetAny (), 9999));
  sinkApp[user_i] = sinkHelper.Install (staNode);
  packetSink[user_i] = StaticCast<PacketSink> (sinkApp[user_i].Get (0));
  sinkApp[user_i].Start (Seconds (0.0));   
  sinkApp[user_i].Stop (Seconds (simulationTime));

  /* Install TCP/UDP Transmitter on the DMG AP for TX */
  OnOffHelper src (socketType, InetSocketAddress (sta_address, 9999));
  ...
  srcApp[user_i] = src.Install (apNode);
  onoff[user_i] = StaticCast<OnOffApplication> (srcApp[user_i].Get (0));
  srcApp[user_i].Start (Seconds (2.0));
  srcApp[user_i].Stop (Seconds (simulationTime));
}

void
CalculateThroughput (string file_dir, NodeContainer allNodes, int ue_num)
{
  double thr[ue_num] = {0};
  double max_snr[ue_num] = {0};
  for (int i = 0; i < ue_num; i++) {
    ...
    thr[i] = CalculateSingleStreamThroughput (packetSink[i], totalRx[i], throughput[i]);
  ...
  allThrFile.close();
  Simulator::Schedule (MilliSeconds (100), &CalculateThroughput, file_dir, allNodes, ue_num);
}



void 
CreateFile (string file_dir,NodeContainer allNodes,int ue_num)
{
  ...
  if (activateApp)
    {
      ...
      for (int i = 0; i < user_num; i++)
        {
          InstallPacketSink (dataRate, allNodes.Get (0), allNodes.Get (i+1), staInterfaces.GetAddress (i), socketType, simulationTime, i);
        }
  ...
  if (activateApp)
    {
      ...
      Simulator::Schedule (Seconds (2.1), &CreateFile, file_dir, allNodes, user_num);
    
    }
  /* Schedule many TXSS CBAPs during the data transmission interval. */
  if (RT_enable)
    {
      Simulator::Schedule (Seconds (2.1), &RapidBeamTraining);
    }
      ...
      for (int i = 0; i < user_num; i++)
        {
          ...
          std::cout << "  Throughput: " << packetSink[i]->GetTotalRx () * 8.0 / ((simulationTime - 1) * 1e6) << " Mbps" << std::endl;
